...
Each array element is now accompanied by 64-bit metadata, which contains the essential round number to prevent ABA
...
Another difference from the original Multi-Array Queues is that the writer/reader positions now point to
...
But competing in the premier league of performance was not the main purpose of this work.
The performance-limiting factors of this Queue are not the algorithms themselves,
...
Besides the raw performance, the latency distribution is an important factor too.
